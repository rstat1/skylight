...
//	public function parse($temp_path, $temp_part, $tags, $replinfo, $toparse, $inc_output, $isfile)
	public static function getTags()
	{
		...
		$result = Database::sql_query("SELECT * FROM categories");
		if (Database::error() != ""){echo "Problem encountered! ";}
		else 
		{			
	...
	{		
		$result = Database::sql_query("SELECT * FROM posts WHERE `id` LIKE '$id'");
	...
	{		
		...
		else{$result = Database::sql_query("SELECT * FROM posts WHERE `category` LIKE '$tag' ORDER by `id` DESC LIMIT 5");}
		if (Database::error() != ""){echo "Problem encountered! ";}
		else 
		{			
	...
	{	
		...
		{	
			if (mysql_num_rows($result) > 0)
			{
				if (mysql_num_rows($result) < 5) {$i = 5;}
				else {$i = 1;}
				while($title = mysql_fetch_assoc($result))
				{												
					$id[] = $title['id'];
