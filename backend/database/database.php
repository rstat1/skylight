...
class Database
{
	...
	{
		global $config;
		if (!isset(self::$connect_id)) {
			self::$connect_id = mysqli_connect($config['db-server'], $config['db-user'], $config['db-pass']);
		}
		if (self::$connect_id) {
			...
			if ($dbsel == true) {
				...
				trigger_error(htmlentities(mysqli_error(self::$connect_id)), E_USER_ERROR);
			}
		} else {
			trigger_error(htmlentities(mysqli_error(self::$connect_id)), E_USER_ERROR);
		}
	}
	public static function get($query, $cache, $table, $forceCacheUpdate = false)
	{
		global $config;
		$cacheHash = base64_encode($query);
		if ($table != NULL) {
			$filename = $table . "_$cacheHash.php";
		} else {
			$filename = "sql_$cacheHash.php";
		}
	...
	public static function ResultCount($queryData)
	{
		if (!is_array($queryData)) {
			trigger_error("ResultCount needs an array", E_USER_ERROR);
		}
		return $queryData[0];
	}
		...
		if ($data != NULL && $table != NULL) {
			if (is_array($data)) {
				...
				if ($query_result == TRUE) {
					...
					trigger_error(mysqli_error(self::$connect_id));
				}
			} else {
				trigger_error("Function expects first argument to be an array.");
			}
		} else {
			trigger_error("This function requires data and table name.");
		}
		return false;
	}
	public static function update($data, $table, $keyfield)
	{
		...
		if ($data != NULL && $table != NULL) {
			...
			trigger_error("This function requires data, a table name and a key field.");
		}
	}
		...
		if ($where != NULL && $table != NULL) {
			if (is_array($where)) {
				...
				if ($query_result) {
					...
					trigger_error(mysqli_error(self::$connect_id) . " " . $finalQuery);
					die();
				}
			} else {
				trigger_error("Function expects first argument to be an array.");
			...
			trigger_error("This function requires some form of criteria so it knows what to delete and a table name so it knows where to delete from.");
		}
	}
	public static function escape_string($str)
	{
		self::connect();
		return mysqli_real_escape_string(self::$connect_id, $str);
	}
